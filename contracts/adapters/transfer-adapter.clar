...
(define-constant DAO_CORE .dao-core)
(define-constant TREASURY .dao-treasury)
  ...
  (begin
    ...
    (let ((kind (get kind payload)))
      (if (is-eq kind "stx-transfer")
        (contract-call? .dao-treasury execute-stx-transfer (get amount payload) (get recipient payload))
